<template>
  ...
  <div v-for="(product, ind) in this.$store.state.products" :key="ind">
    <v-card
      ...
      <v-card-actions style=" border-bottom: #777;">
        <h3 style="float: left; margin-right: 100px;">{{ product.name }} - {{ product.cost }}</h3>
        ...
        <div>
          <div style="float: left; margin-right: 15px;">
            ...
            <v-radio-group v-for="(buyer, ind) in this.$store.state.buyers" :key="ind" v-model="product.buyer">
              <v-radio  :label="buyer.name" :value="ind"></v-radio>
            </v-radio-group>
  ...
  <v-text-field @blur="this.$store.state.tempProduct.name = $event.target.value" v-model="this.$store.state.tempProduct.name">
  </v-text-field>
  <v-text-field @blur="this.$store.state.tempProduct.cost = $event.target.value" v-model="this.$store.state.tempProduct.cost">
  </v-text-field>
